...
const Deliverables = () => {
  const { deliverables, projects, addDeliverable, deleteDeliverable } = useData();
  ...
  const handleDelete = (index) => {
    ...
    if (confirm(`"${deliverable.name}" 산출물을 삭제하시겠습니까?`)) {
      ...
      deleteDeliverable(originalIndex);
    }
  };

  // 프로젝트별 필터링 및 날짜 정렬
  const filteredDeliverables = deliverables
    .filter(deliverable => selectedProject === '전체' || deliverable.project === selectedProject)
    .sort((a, b) => new Date(a.date) - new Date(b.date));

  return (
    <div className="p-6">
      <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <h1 className="main-title">산출물</h1>
      ...
      {filteredDeliverables.length > 0 ? (
        <div className="timeline ml-4">
          {filteredDeliverables.map((deliverable, index) => (
            <div key={index} className="deliverable-item bg-white p-4 rounded-md shadow-md hover:translate-y-[-2px] transition-all">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-semibold text-gray-900 text-base">{deliverable.name}</h3>
                  <p className="text-sm text-gray-600">
                    {deliverable.project} · {formatDate(deliverable.date)}
                  </p>
                  <p className="text-sm mt-1">
                    <StatusBadge status={deliverable.status} />
                  </p>
                  {deliverable.description && (
                    <p className="text-sm text-gray-600 mt-3">{deliverable.description}</p>
                  )}
                </div>
                <button
        ...
        <div className="bg-white rounded-xl shadow-md p-12 text-center">
          <p className="text-gray-500 text-lg">산출물이 없습니다</p>
          <p className="text-gray-400 text-sm mt-2">새로운 산출물을 추가해보세요</p>
        </div>
      )}
